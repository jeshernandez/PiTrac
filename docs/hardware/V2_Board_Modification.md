...
## Modification Steps
	...
	1.  Attach trigger + and trigger ground wires to the MOSFET module:  
		...
		c.  Solder the black wire to the pad labeled "GND" on the MOSFET module (your labelling might be different).  See the close-up image of the module here:  ![MOSFET Module Closeup]({{ '/assets/images/hardware/MOSFET_Module_Closeup.png' | relative_url }}).
	...
	5.  Connect the Module's Trigger Wires to the Connector Board
		...
		d.  A trick here is to exploit the fact that Pin 1 is an unused Pin on the chip and is not electrically connected to anything (at least for the component we've been using).  So, you can solder the trigger wire to both Pins 1 and 2 (with the wire coming in from the right (edge) side of the board.  See this close up: ![U9 Pin Soldering Closeup]({{ '/assets/images/hardware/Soldering_Trigger_Wire_To_U9.png' | relative_url }}).
