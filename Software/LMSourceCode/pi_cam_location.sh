#!/bin/bash
rm -f discover_media.txt discover_device.txt discover_result.txt /home/pitracuser/Dev/PiTrac/Software/LMSourceCode/pi_cam_location.txt
for ((m = 0; m <= 5; ++m))
    do
       ...
       if  grep imx discover_media.txt > /dev/null;  then  cat discover_result.txt >> /home/pitracuser/Dev/PiTrac/Software/LMSourceCode/pi_cam_location.txt;  fi
            done
