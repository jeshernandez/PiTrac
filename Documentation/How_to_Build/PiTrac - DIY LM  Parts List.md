...
| 1 | [DEPRECATED] We used to suggest a Raspberry Pi 4 Model B](https://www.raspberrypi.com/products/raspberry-pi-4-model-b/) and power supply, 4 GB minimum or even (for a little more money) a second Pi 5 instead of the Pi 4\.  However, the current version of a PiTrac really only requires only a single Pi 5.  The "Single-Pi" version is pretty new and may still have some problems.  However, it appears to be working and the code has been checked in.  So, if you're building a new PiTrac, you probably don't need a second Pi. |  |
...
| 2  | [Raspberry Pi Global Shutter Camera – CS Lens Mount](https://www.adafruit.com/product/5702)&nbsp; Alternatively, the following camera also seems to work with PiTrac: [innomaker GS Camera Module with IMX296 Mono Sensor ](https://www.amazon.com/gp/product/B093BY2TK2/ref=ewc_pr_img_1?th=1).  This camera should be much easier to work with than the 'official' Raspberry Pi camera, because it doesn't require any soldering (or voiding the warranty).  At this point, the InnoMaker is probably the best choice even though it's not as well tested as the official Pi GS camera.  The Pi cameras do not come with lenses (see below), but the Innomakers come with M12 lenses.  The included M12 3.6mm lenses are not tested out with PiTrac yet, but may work if the ball is close.  But, you'll still probably want to get the 6mm lenses we recommend below. The Innomakers also come with a short ribbon cable to connect the Pi 5 to the camera, but it's probably only long enough to work with one of the two PiTrac cameras, so you'll want a longer one. |
| :---- | :---- | :---- |
...
| 1 | [1" x 1", Optical Cast Plastic IR Longpass Filter](https://www.edmundoptics.com/p/1quot-x-1quot-optical-cast-plastic-ir-longpass-filter/5421/) (1.5mm nominal thickness) | Blocks visible light below ~730nm from entering the Camera 2 sensor.  NOTE:  You want to make sure that the filter you get does not get rid of near-infrared 730nm light, because that's the type of infrared the project is using.  You can usually find a filter that cuts just the visible light below 730nm.  If you get an 850nm filter, things won't work. |
|  |  |  |
